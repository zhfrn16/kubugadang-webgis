...
class UnitHomestay extends ResourcePresenter
{
    ...
    public function update($id = null)
    {
        $request = $this->request->getPost();
        $requestData = [
            'unit_name' => $request['editNama_unit'],
    ...
    public function delete ($homestay_id=null, $unit_type=null, $unit_number=null, $unit_homestay_id=null)
    {
        ...
        if ($deleteGUD){
            $array2 = array('homestay_id' => $homestay_id, 'unit_type' => $unit_type,'unit_number' => $unit_number, 'unit_name'=> $unit_name,'description'=> $description);
            $unitHomestay = $this->unitHomestayModel->where($array2)->find();
            $deleteUH= $this->unitHomestayModel->delete_unit($array2);

            if ($deleteUH) {
                session()->setFlashdata('pesan', 'Unit "'.$unit_name.' '.$unit_number.'" Homestay "'.$homestay_id.'" Deleted Successfully.');
