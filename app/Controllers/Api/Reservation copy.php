...
class Reservation extends ResourceController
{
    ...
    //         $qualified_units = []; // Inisialisasi array untuk menyimpan unit yang memenuhi syarat di rumah gadang tertentu

    //         foreach ($list_unit as $unit) {
    //             // Pilih unit yang memenuhi syarat
    //             if ($totalPeople > 0) {
    //                 $unit_capacity = min($totalPeople, $unit['capacity']); // Ambil kapasitas unit atau sisa totalPeople, yang lebih kecil
    //                 $qualified_units[] = [
    ...
    //function chooseHome fix
    public function chooseHome2()
    {
        $request = $this->request->getPost();
        $checkInDate = $request['checkInDate'];
        $totalPeople = $request['totalPeople'];
    ...
    public function statistictersedia()
    {
        ...
        foreach ($list_homestay as $homestay) {
            ...
            // Retrieve unit details with total reservations
            $list_units = $this->unitHomestayModel->unit_tersedia_lagi($homestay_id, $unit_type, $unit_number, $checkInDate, $totalPeople);
            ...
            foreach ($list_units as $unit) {
                ...
                if ($totalPeople > 0 && $unit['unit_remaining'] > 0) {
                    ...
                    // Jika sisa tamu yang belum terpenuhi cukup untuk memilih unit ini
                    if ($totalPeople - $unit_capacity >= 0) {
                        // Tambahkan unit yang dipilih ke dalam array
                        $units_selected[] = [
                            'homestay_id' => $unit['homestay_id'],
                            'unit_type' => $unit['unit_type'],
                            ...
                            'totalPeopleRemaining' => $totalPeople - $unit_capacity,
                            'url' => $unit['url'],
                            // 'facility_units' => $fc,
                            // Tambahan data lainnya sesuai kebutuhan
                        ];
                        ...
                        // Jika jumlah tamu sudah mencukupi, keluar dari loop unit
                        if ($totalPeople <= 0) {
                            break;
                        }
                    }
    ...
    // public function statisticbookingcustom()
    // {
    ...
    //         if ($totalPeople < 3) {
    //             $list_unit = $this->unitHomestayModel->get_unit_homestay_with_gallery_custom_small($homestay_id, $checkInDate, $totalPeople)->getResultArray();
    //         } else if ($totalPeople < 11) {
    //             $list_unit = $this->unitHomestayModel->get_unit_homestay_with_gallery_custom_medium($homestay_id, $checkInDate, $totalPeople)->getResultArray();
    //         } else {
    //             $list_unit = $this->unitHomestayModel->get_unit_homestay_with_gallery_large($homestay_id, $checkInDate, $totalPeople)->getResultArray();
    ...
    //         foreach ($list_unit as $unit) {
    //             if ($totalPeople > 0) {
    //                 // Ubah bagian ini untuk menangani kamar dengan kapasitas lebih besar dari totalPeople
    //                 $unit_capacity = ($totalPeople >= $unit['capacity']) ? $unit['capacity'] : $totalPeople;
    //                 $qualified_units[] = [
    //                     'homestay_id' => $unit['homestay_id'],
    //                     'unit_type' => $unit['unit_type'],
    //                     'unit_number' => $unit['unit_number'],
