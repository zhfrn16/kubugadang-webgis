...
class EventModel extends Model
{
    ...
    protected $allowedFields    = [
        'id', 'name', 'type', 'description', 'event_start', 'event_end', 'price',
    ...
    public function get_list_event_api() {
        $coords = "ST_Y(ST_Centroid({$this->table}.geom)) AS lat, ST_X(ST_Centroid({$this->table}.geom)) AS lng";
        $columns = "{$this->table}.id,{$this->table}.name,{$this->table}.type as category, {$this->table}.event_start, {$this->table}.event_end, {$this->table}.description,
                        {$this->table}.price,{$this->table}.contact_person,{$this->table}.video_url";
        ...
        public function get_list_ev_api() {
            $coords = "ST_Y(ST_Centroid({$this->table}.geom)) AS lat, ST_X(ST_Centroid({$this->table}.geom)) AS lng";
            $columns = "{$this->table}.id,{$this->table}.name,{$this->table}.type as category,{$this->table}.event_start, {$this->table}.event_end,{$this->table}.description,{$this->table}.price,{$this->table}.contact_person,{$this->table}.video_url";
            // $vilGeom = "village.id = '1' AND ST_Contains(village.geom, {$this->table}.geom)";
            $query = $this->db->table($this->table)
                ->select("{$columns}, {$coords}")
                ->from('village')
