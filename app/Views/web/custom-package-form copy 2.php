...
function activityOptions() {
        ...
        if (table.getAttribute('data-day') === selectedDay) { // Cek tabel berdasarkan Day
            ...
            rows.forEach((row) => {
                ...
                if (!isNaN(activityValue)) {
                    lastActivity = Math.max(lastActivity, activityValue); // Update lastActivity
                }
            });
        }
    });
    ...
    if (activityInput) {
        activityInput.value = lastActivity + 1;
    }

    console.log("Selected Day: ", selectedDay);
console.log("Tabel ditemukan: ", tables);
console.log("Last Activity Sebelum Update: ", lastActivity);
